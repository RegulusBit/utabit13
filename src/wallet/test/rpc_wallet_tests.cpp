// Copyright (c) 2013-2015 The Utabit Core developers
...
BOOST_AUTO_TEST_CASE(rpc_wallet)
{
    ...
    UniValue r;
    CPubKey demoPubkey;
    CUtabitAddress demoAddress;
    UniValue retValue;
    string strAccount = "walletDemoAccount";
    CUtabitAddress setaccountDemoAddress;
    {
        LOCK(pwalletMain->cs_wallet);

        demoPubkey = pwalletMain->GenerateNewKey();
        demoAddress = CUtabitAddress(CTxDestination(demoPubkey.GetID()));
        ...
        setaccountDemoAddress = CUtabitAddress(CTxDestination(setaccountDemoPubkey.GetID()));
    }
    /*********************************
     * 			setaccount
     *********************************/
    ...
    BOOST_CHECK(CUtabitAddress(retValue.get_str()).Get() == demoAddress.Get());
    ...
    BOOST_CHECK(CUtabitAddress(arr[0].get_str()).Get() == demoAddress.Get());
