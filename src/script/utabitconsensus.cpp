...
inline int set_error(utabitconsensus_error* ret, utabitconsensus_error serror)
{
    if (ret)
        *ret = serror;
    return 0;
                                    ...
                                    unsigned int nIn, unsigned int flags, utabitconsensus_error* err)
{
    try {
        ...
        stream >> tx;
        if (nIn >= tx.vin.size())
            return set_error(err, utabitconsensus_ERR_TX_INDEX);
...
unsigned int utabitconsensus_version()
{
    ...
    return UTABITCONSENSUS_API_VER;
}
